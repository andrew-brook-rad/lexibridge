...
async function test() {
  console.log('Starting comprehensive verification test (Session 38)...');
  ...
  try {
    ...
    await page.goto('http://localhost:3003', { waitUntil: 'networkidle0', timeout: 60000 });
    await new Promise(r => setTimeout(r, 2000));
    await page.screenshot({ path: 'screenshots/session38-01-initial.png' });
    ...
    if (genesisBtn) {
      ...
      await page.screenshot({ path: 'screenshots/session38-02-genesis-loaded.png' });
    ...
    await page.screenshot({ path: 'screenshots/session38-03-pdf-view.png' });
    ...
    if (singlePageBtn) {
      ...
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session38-04-single-page.png' });
    ...
    if (bookSpreadBtn) {
      ...
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session38-05-book-spread.png' });
    ...
    if (editModeBtn) {
      ...
      await page.screenshot({ path: 'screenshots/session38-06-edit-mode.png' });
    ...
    if (pdfBtn) {
      await pdfBtn.click();
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session38-07-final-pdf.png' });
    }
    ...
    await page.screenshot({ path: 'screenshots/session38-error.png' }).catch(() => {});
    await browser.close();
    process.exit(1);
  }
}
