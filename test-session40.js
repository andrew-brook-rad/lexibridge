...
async function test() {
  console.log('Starting comprehensive verification test (Session 40)...');
  ...
  try {
    ...
    await page.screenshot({ path: 'screenshots/session40-01-initial.png' });
    ...
    if (genesisBtn) {
      ...
      await page.screenshot({ path: 'screenshots/session40-02-genesis-loaded.png' });
    ...
    await page.screenshot({ path: 'screenshots/session40-03-pdf-view.png' });
    ...
    if (singlePageBtn) {
      ...
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session40-04-single-page.png' });
    ...
    if (bookSpreadBtn) {
      ...
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session40-05-book-spread.png' });
    ...
    if (editModeBtn) {
      ...
      await page.screenshot({ path: 'screenshots/session40-06-edit-mode.png' });
    ...
    if (pdfBtn) {
      await pdfBtn.click();
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session40-07-final-pdf.png' });
    }
    ...
    await page.screenshot({ path: 'screenshots/session40-error.png' }).catch(() => {});
    await browser.close();
    process.exit(1);
  }
}
