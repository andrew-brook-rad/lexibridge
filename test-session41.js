...
async function test() {
  console.log('Starting comprehensive verification test (Session 41)...');
  ...
  try {
    ...
    await page.goto('http://localhost:3006', { waitUntil: 'networkidle0', timeout: 60000 });
    await new Promise(r => setTimeout(r, 2000));
    await page.screenshot({ path: 'screenshots/session41-01-initial.png' });
    ...
    if (genesisBtn) {
      ...
      await page.screenshot({ path: 'screenshots/session41-02-genesis-loaded.png' });
    ...
    await page.screenshot({ path: 'screenshots/session41-03-pdf-view.png' });
    ...
    if (singlePageBtn) {
      ...
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session41-04-single-page.png' });
    ...
    if (bookSpreadBtn) {
      ...
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session41-05-book-spread.png' });
    ...
    if (editModeBtn) {
      ...
      await page.screenshot({ path: 'screenshots/session41-06-edit-mode.png' });
    ...
    if (pdfBtn) {
      await pdfBtn.click();
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session41-07-final-pdf.png' });
    }
    ...
    await page.screenshot({ path: 'screenshots/session41-error.png' }).catch(() => {});
    await browser.close();
    process.exit(1);
  }
}
