...
async function test() {
  console.log('Starting comprehensive verification test (Session 42)...');
  ...
  try {
    ...
    await page.screenshot({ path: 'screenshots/session42-01-initial.png' });
    ...
    if (genesisBtn) {
      ...
      await page.screenshot({ path: 'screenshots/session42-02-genesis-loaded.png' });
    ...
    await page.screenshot({ path: 'screenshots/session42-03-pdf-view.png' });
    ...
    if (singlePageBtn) {
      ...
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session42-04-single-page.png' });
    ...
    if (bookSpreadBtn) {
      ...
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session42-05-book-spread.png' });
    ...
    if (editModeBtn) {
      ...
      await page.screenshot({ path: 'screenshots/session42-06-edit-mode.png' });
    ...
    if (pdfBtn) {
      await pdfBtn.click();
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session42-07-final-pdf.png' });
    }
    ...
    await page.screenshot({ path: 'screenshots/session42-error.png' }).catch(() => {});
    await browser.close();
    process.exit(1);
  }
}
