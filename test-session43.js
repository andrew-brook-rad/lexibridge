...
(async () => {
  ...
  await page.screenshot({ path: 'screenshots/session43-01-initial.png' });
  ...
  if (genesisButton) {
    ...
    await page.screenshot({ path: 'screenshots/session43-02-genesis-loaded.png' });
  ...
  for (const btn of allButtons) {
    ...
    if (text && text.includes('Single')) {
      ...
      await page.screenshot({ path: 'screenshots/session43-03-single-page.png' });
  ...
  for (const btn of allButtons) {
    ...
    if (text && text.includes('Book')) {
      ...
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session43-04-book-spread.png' });
  ...
  for (const btn of allButtons) {
    ...
    if (text && text.includes('Edit')) {
      ...
      await page.screenshot({ path: 'screenshots/session43-05-edit-mode.png' });
  ...
  for (const btn of allButtons) {
    ...
    if (text && text.includes('PDF')) {
      ...
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session43-06-final-pdf.png' });
