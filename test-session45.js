...
(async () => {
  ...
  // Navigate to app
  await page.goto('http://localhost:3008', { waitUntil: 'networkidle0', timeout: 30000 });
  console.log('Page loaded');

  // Take initial screenshot
  await page.screenshot({ path: 'screenshots/session45-01-initial.png' });
  ...
  if (genesisButton) {
    ...
    await page.screenshot({ path: 'screenshots/session45-02-genesis-loaded.png' });
  ...
  for (const btn of allButtons) {
    ...
    if (text && text.includes('Single')) {
      ...
      await page.screenshot({ path: 'screenshots/session45-03-single-page.png' });
  ...
  for (const btn of allButtons) {
    ...
    if (text && text.includes('Book')) {
      ...
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session45-04-book-spread.png' });
  ...
  for (const btn of allButtons) {
    ...
    if (text && text.includes('Edit')) {
      ...
      await page.screenshot({ path: 'screenshots/session45-05-edit-mode.png' });
  ...
  for (const btn of allButtons) {
    ...
    if (text && text.includes('PDF')) {
      ...
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: 'screenshots/session45-06-final-pdf.png' });
