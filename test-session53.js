...
(async () => {
  console.log('Starting verification test (Session 53)...');
  ...
  try {
    ...
    await page.goto('http://localhost:3006', { waitUntil: 'networkidle0', timeout: 30000 });
    await page.screenshot({ path: path.join(screenshotDir, 'session53-01-initial.png'), fullPage: false });
    ...
    await page.screenshot({ path: path.join(screenshotDir, 'session53-02-genesis-loaded.png'), fullPage: false });
    ...
    await page.screenshot({ path: path.join(screenshotDir, 'session53-03-single-page.png'), fullPage: false });
    ...
    await page.screenshot({ path: path.join(screenshotDir, 'session53-04-book-spread.png'), fullPage: false });
    ...
    await page.screenshot({ path: path.join(screenshotDir, 'session53-05-edit-mode.png'), fullPage: false });
    ...
    await page.screenshot({ path: path.join(screenshotDir, 'session53-06-final-pdf.png'), fullPage: false });
    console.log('Final PDF view: YES');

    console.log('\n========================================');
    console.log('ALL TESTS PASSED - Session 53 Verification Complete');
    console.log('========================================');

  } catch (error) {
    console.error('Test error:', error.message);
    await page.screenshot({ path: path.join(screenshotDir, 'session53-error.png'), fullPage: true });
  } finally {
    await browser.close();
  }
})();
